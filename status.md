...
### 1d. 八重瀬町LP（`cursor-test/yaese.html`）
...
### 1e. 豊見城市LP（`cursor-test/tomigusuku.html`）
...
### 1f. 浦添市家庭LP（`cursor-test/urasoe.html`）
...
2. **八重瀬町LPのSearch Console反映状況確認**（sitemap再送信・URL検査の再実行は不要）
