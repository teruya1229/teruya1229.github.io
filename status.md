...
- **南風原町LP `haebaru.html` 既存刷新・公開確認OK（2026-06-10）**。新規作成ではなく既存SEO資産・内部リンク資産を活かす刷新で対応（`7098e05` / `e0614ac` / `ab4f1f5` / `5b4949f`）。FAQ表示8問とJSON-LD8問を一致。スマホ固定CTA軽微CSS修正済み
- **与那原町LP `yonabaru.html` 既存刷新・公開確認OK（2026-06-10）**。新規作成ではなく既存SEO資産・内部リンク資産を活かす刷新で対応（`9e74947`）。FAQ表示8問とJSON-LD8問を一致
- **八重瀬町LP `yaese.html` 既存刷新・公開確認OK（2026-06-10）**。新規作成ではなく既存SEO資産・内部リンク資産を活かす刷新で対応（`d4c299a`）。FAQ表示8問とJSON-LD8問を一致
- **豊見城市LP `tomigusuku.html` 既存刷新・公開確認OK（2026-06-10）**。新規作成ではなく既存SEO資産・内部リンク資産を活かす刷新で対応（`8322aba` / `b73db60`）
- **南城市LP `nanjo.html` 既存刷新・公開確認OK（2026-06-10）**。新規作成ではなく既存SEO資産を活かす刷新で対応
- **浦添市家庭LP `urasoe.html` 公開確認OK（2026-06-10）**。市町村別LP量産の1本目完了
...
- **中部家庭LP `central.html` 公開準備完了**。Search Console は時間を置いて反映確認
...
### 1h. 浦添市家庭LP（`cursor-test/urasoe.html`）
...
1. **次地域LPの着手前判断**（都度SEO調査）
   ...
   - いきなり刷新せず、既存確認・sitemap・内部リンク・現行標準との差分を先に確認
   - その結果で「既存刷新」か「新規作成」か判断する
   - 糸満市は人口が多くLP経由の反応もあるため優先度は高い。海風・潮風・湿気・戸建て/団地/新興住宅・西崎/潮崎/兼城/真栄里などの生活圏・完全分解需要を確認
   - SERP詳細は長期正本に残さず、判断ルールと作業結果のみ記録
2. **南風原町LPのSearch Console反映状況確認**（sitemap再送信・URL検査の再実行は不要）
   - 時間を置いて `haebaru.html` のインデックス反映状況を確認
3. **与那原町LPのSearch Console反映状況確認**（sitemap再送信・URL検査の再実行は不要）
...
6. **浦添LPのSearch Console反映状況確認**
   - `https://teruya1229.github.io/cursor-test/urasoe.html` と中部LP `central.html` もあわせて確認
7. **市町村別LP量産の判断**（上記確認後）
   - ゴール達成型プロンプトを基本にする（composer-2.5・南城市・八重瀬町・与那原町・南風原町刷新で再現性確認済み）
8. **南城市LPヤモリ・虫・カバー内汚れ追記の公開確認**（別途 `ef5e688`）。必要なら記録を補足
9. **note有料記事への追記**
   - 「ゴール達成型で既存LP刷新まで進められた」事例として追記候補
   - 「composer-2.5でもゴール達成型が通用した」検証結果も追記候補
10. **Google広告の小額テスト設計**（表示確認OKなら進む）
   - まずは完全分解LP向けにキャンペーン作成
   - 1日500〜1,000円程度で開始
11. **広告番頭への日次入力運用**
   - 広告番頭に数値入力（Claude in Chrome 運用）
   - 7日分の数字で継続 / 改善 / 停止を判断
...
- 沖縄市ページ / 宜野湾市ページ / うるま市ページの量産（浦添LPの反応・表示確認後）
