...
住宅設備3LP（エアコン買換え `/mi-bcs/aircon-replacement/`、トイレ `/toilet/`、ウォシュレット `/washlet/`）は commit `f56f098` をCRO再テスト基準点とする。当面これらのLPは追加修正しない。広告設定は変更せず、同じ広告条件で実問い合わせを観測する。
...
**【residential 3LP CRO 公開確認完了】**

- 日時：2026年9月20日
- 対象リポジトリ：`C:\dev\bc-service\teruya1229-github-io`
- 実装commit：`f56f098c31323522561116a5fb589a1293378c17`（`feat: improve residential LP conversion flow`）
- 変更ファイル：`mi-bcs/aircon-replacement/index.html` / `toilet/index.html` / `toilet/assets/css/toilet.css` / `washlet/index.html` / `washlet/assets/css/washlet.css`
- 公開URL（cache buster付きで確認済み）：
  - `https://teruya1229.github.io/mi-bcs/aircon-replacement/?v=f56f098`
  - `https://teruya1229.github.io/toilet/?v=f56f098`
  - `https://teruya1229.github.io/washlet/?v=f56f098`
- LINE：`https://lin.ee/Q1s3uqV` 維持
- 電話：`050-1724-1338` / `tel:05017241338` 維持
- GA4：`G-X8LKLFDL3G` 維持。イベント名未変更
- 広告設定：未変更
- 375 / 390 / 768 / 1440px：横スクロールなし、JS errorなし、公開URLの画像HTTP 404なし
- この3LPの今回状態をCRO再テスト基準点とする。当面LP追加修正なし。実問い合わせを観測する

**【エアコン買換え `/mi-bcs/aircon-replacement/`】**

- FVで畳数別価格：6畳 99,800円〜 / 8畳 119,800円〜 / 10畳 129,800円〜 / 14畳 149,800円〜
- 何込み：本体＋標準取付＋既存エアコン撤去＋リサイクル＋工事5年保証
- 詳細価格ブロックはFV直後
- 清掃・完全分解の説明は削除せず後段へ移動（技術根拠）
- LINE写真送信後の案内：本命1台＋比較1台＋標準工事込み総額の目安
- エアコン買換えの工事5年保証は既存の工事保証。トイレ／ウォシュレットの商品長期保証と混同しない

**【トイレ `/toilet/`】**

- 梅 149,800円〜 / 竹 179,800円〜 / 松 279,800円〜
- 竹の価値：梅＋30,000円で TOTO KQ＋瞬間式ウォシュレット TCF8FK58
- 施工は案件により照屋本人または協力施工者。相談窓口はBCサービス
- 対象商品は有料で5年・8年・10年の長期保証を選択可能
- これは商品側の長期保証である。「工事保証10年」ではない
- 商品は業務用仕入れルートから手配（一般客向けにBFC名は出さない）
- LINE写真4枚から、排水方式・設置可否・おすすめプラン・追加工事の有無・正式金額を確認

**【ウォシュレット `/washlet/`】**

- TOTO TCF8FK58、54,800円税込
- 本体＋標準取付＋既存便座撤去・処分込み
- スマホFV優先順：型番 → 価格 → 何込み → CTA → 設置イメージ
- 施工は案件により照屋本人または協力施工者
- 対象商品は有料で5年・8年・10年の長期保証（商品保証。工事保証期間ではない）
- LINE写真4枚から、取付可否・54,800円適用可否・追加工事の有無を確認
...
ハウスクリーニングLPの指定3点を最小差分で修正。実装・ローカル確認・push後、公開URLをユーザー本人が目視確認し「OK」。

**【ピンポイント修正完了】**

- 日時：2026年8月13日
- 実装commit：`2945524bfcf40d59d4c54707d46349acad9166c7`（`fix: refine house cleaning service cards and pricing`）
- 変更ファイル：`house-cleaning/index.html`（`style.css` は変更なし）
- 公開URL：`https://teruya1229.github.io/house-cleaning/`
- 公開確認URL：`https://teruya1229.github.io/house-cleaning/?v=2945524`

**【修正内容】**

- 水まわりセクションのキッチン写真なし紹介カードを `<article>` 単位で削除
- キッチンサービス自体は廃止していない。水まわり5点セット・セット料金・単品料金「キッチン 14,300円」は維持
- エアコン欄へ家庭LPの1枚型ビフォーアフター画像 `../cursor-test/images/case1-before-after.jpg` を反映（家庭LP側の画像・HTMLは未変更）
- セット料金3枚（キッチン＋レンジフード／浴室＋トイレ＋洗面台／浴室＋キッチン＋レンジフード）へ「一般的な汚れ」「軽い汚れの最安」を追加。上限料金は変更なし
...
広告番頭 `ops/ad-bantou/` に Phase 1「運用ダッシュボード改善」を実装し、GitHub Pages へ公開完了。

**【公開完了】**

- 日時：2026年6月15日
- 対象URL：`https://teruya1229.github.io/ops/ad-bantou/`
- commit：`c861b45`（広告番頭 Phase 1 ダッシュボード改善）
- 変更ファイル：`ops/ad-bantou/index.html` / `ops/ad-bantou/app.js` / `ops/ad-bantou/style.css`
...
- 最終保存日時表示
- 異常・注意バナー
- 昨日サマリーKPIカード6枚（広告費・クリック・問い合わせ・成約・CPA・ROAS）
- 日別ログフィルタ（日付 / LP種別 / 判定 / 入力状態）
- スマホ用ログカード表示（520px以下）
- CSS変数による管理画面風デザイン整理

**【互換性】**

- 既存localStorageキー3つはスキーマ変更なし（`bcAdBantouDailyLogs` / `bcAdBantouPreflightChecks` / `bcAdBantouCampaignSetupChecks`）
- 新規キー：`bcAdBantouLastSaveMeta`（最終保存日時メタのみ）
- 既存コアロジック維持：`decide()` / `computeMetrics()` / `parseBrowserBantouJson()`

**【確認済み】**

- push成功 / GitHub Pages公開確認（HTTP 200 / JSエラーなし）
- 375pxスマホ表示OK / テーブル・カード二重表示なし
- 公開確認環境ではlocalStorage実データは見えないため、ユーザー本人のChromeで最終確認予定
- GitHub Pages反映直後はCDNキャッシュで旧版が出る場合あり（Ctrl+Shift+R）

**【未着手】**

- 広告番頭 Phase 2（ミニチャート・export/import 等）はまだ着手しない

### 2026-06-15（広告番頭：21時チェックUX改善）

広告番頭 `ops/ad-bantou/` に「今日の21時チェック」専用パネルを最上部に追加。クリップボード読込・一括JSONプレビュー・2件まとめて保存・重複検知（上書き/スキップ）対応。設定チェック類は折りたたみに移動。
...
- **2026-06-15 Phase 1 運用ダッシュボード改善 公開完了**（`c861b45`）：運用ダッシュボード・KPIカード・異常バナー・ログフィルタ・スマホカード表示・最終保存日時・CSS変数整理
- 公開URL：`https://teruya1229.github.io/ops/ad-bantou/`
- 日次入力 / 自動計算 / 自動判定（継続・改善・停止候補・データ不足）/ 日別ログ / LP別集計
- **21時チェック専用パネル**（クリップボード読込・2件まとめて保存・重複上書き/スキップ）
- **ブラウザー番頭JSON読み込み**（単体・daily-check一括プレビュー）
...
- localStorage 保存（日次：`bcAdBantouDailyLogs` / 開始前チェック：`bcAdBantouPreflightChecks` / 初期設定チェック：`bcAdBantouCampaignSetupChecks` / 最終保存メタ：`bcAdBantouLastSaveMeta`）
