...
**住宅設備3LP CRO基準点 確定（2026-09-20）**

- 対象：`/mi-bcs/aircon-replacement/` / `/toilet/` / `/washlet/`
- 実装commit：`f56f098c31323522561116a5fb589a1293378c17`（`feat: improve residential LP conversion flow`）
- 公開確認済み。3LPとも今回の状態をCRO再テスト基準点とする
- 当面これらのLPは追加修正しない。広告設定は変更しない。同じ広告条件で実問い合わせを観測する
- LINE：`https://lin.ee/Q1s3uqV` ／ 電話：`050-1724-1338` ／ GA4：`G-X8LKLFDL3G`
- エアコン買換えFV：6畳 99,800円〜 / 8畳 119,800円〜 / 10畳 129,800円〜 / 14畳 149,800円〜。本体＋標準取付＋既存撤去＋リサイクル＋工事5年保証。詳細価格はFV直後。清掃・完全分解説明は後段。LINE後は本命1台＋比較1台＋標準工事込み総額目安
- トイレ：梅 149,800円〜 / 竹 179,800円〜 / 松 279,800円〜。竹は梅＋30,000円で TOTO KQ＋瞬間式ウォシュレット TCF8FK58。施工は照屋本人または協力施工者。窓口はBCサービス。LINE写真4枚で排水方式・設置可否・おすすめプラン・追加工事有無・正式金額を確認
- ウォシュレット：TOTO TCF8FK58 54,800円税込（本体＋標準取付＋既存便座撤去・処分込み）。スマホFVは型番→価格→何込み→CTA→設置イメージ。LINE写真4枚で取付可否・54,800円適用可否・追加工事有無を確認
- トイレ／ウォシュレットの長期保証：対象商品の有料5年・8年・10年（商品側）。「工事保証10年」ではない
- エアコン買換えの工事5年保証は既存の工事保証。商品長期保証と混同しない
- 商品は業務用仕入れルートから手配。一般客向けLPにBFC名は出さない
- **結論：3LP CRO修正は公開済み。次はLP追加修正ではなく、実問い合わせの観測**
...
- 対象：`house-cleaning/index.html` / `style.css`
- 実装commit：`43f6411c0e80b71057acaf097afa5061386065b9`（`fix: refine cleaning photo hierarchy and section layout`）
- 公開URL：`https://teruya1229.github.io/house-cleaning/?v=43f6411`
- 内容：単独大写真2削除／高圧洗浄は外壁BA主役＋料金カード分離／作業事例3件化／床ニス左・ワックス右＋中央揃え／5点セット重複・空白解消
- GitHub Pages：completed / success（run 31315859778）
- Browser番頭確認：PC1440／390px OK。画像404なし、横スクロールなし、計測OK、Consoleエラーなし
- 実機確認未完了（物理スマートフォン未使用）
- **結論：写真構成整理の公開確認まで完了。次は物理スマホ実機確認**
...
- 実機確認未完了（物理スマートフォン未使用）
- **結論：実施工写真の反映・公開確認まで完了。構成整理は上記フェーズを正とする**
...
- 公開URL：`https://teruya1229.github.io/house-cleaning/?v=956170a`
...
- **結論：新規LPの実装・公開確認まで完了。床サービス追加後は上記フェーズを正とする**
...
住宅設備3LP（エアコン買換え／トイレ／ウォシュレット）は `f56f098` をCRO再テスト基準点とし、当面追加修正しない。同じ広告条件で実問い合わせを観測する。新しい実データまたは明確な不具合が出た場合だけ最小修正する。

## 判断基準

- 問題対応・改善作業の進め方は、ルート [`rules.md`](rules.md) の共通ルール（原則ゴール達成型）に従う
- 住宅設備3LP（`/mi-bcs/aircon-replacement/` / `/toilet/` / `/washlet/`）は `f56f098` を基準点とし、当面 **触らない**（実問い合わせ観測を優先。明確な不具合または新しい実データが出たときのみ最小修正）
- トイレ／ウォシュレットの長期保証は対象商品の有料5年・8年・10年（商品側）。「工事保証10年」とは書かない
